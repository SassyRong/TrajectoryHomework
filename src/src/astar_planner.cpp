...
struct GridMap {
    ...
    void markObstacle(double cx, double cy, double radius, double safety_radius = 0.0) {
        ...
        int grid_safety_radius = std::round(safety_radius / grid_resolution);
        // Step 1: 将圆形区域标记为占用
            // your code
        for(int i = std::max(0, grid_cx - grid_radius - grid_safety_radius); i < std::min(width, grid_cx + grid_radius + grid_safety_radius); i++){
            for(int j = std::max(0, grid_cy - grid_radius - grid_safety_radius); j < std::min(height, grid_cy + grid_radius + grid_safety_radius); j++){
                if((i - grid_cx) * (i - grid_cx) + (j - grid_cy) * (j - grid_cy) <= (grid_radius + grid_safety_radius) * (grid_radius + grid_safety_radius)){
                    grid[i][j] = 1;
                }
            }
        }
...
public:
    ...
    void setObstacle(double cx, double cy, double radius, double safety_radius = 0.0) {
        num_of_obs_++;
        grid_map_.markObstacle(cx, cy, radius, safety_radius);
    }
    ...
    std::vector<Eigen::Vector2d> findPath(Eigen::Vector2d start, Eigen::Vector2d goal) {
        ...
        closed_list[gridStart.first][gridStart.second] = true;
        ...
        while (!open_list.empty()) {
            ...
            for (const auto& neighbor : neighbors) {
                ...
                auto new_node = std::make_shared<Node>(neighbor.x, neighbor.y, new_g_cost, new_h_cost, current);
                open_list.push(new_node);
                closed_list[neighbor.x][neighbor.y] = true;
            }
        }
            // 注释结束
            // your code
    ...
    std::vector<Node> getNeighbors(const Node& current) {
        ...
        for (const auto& dir : directions) {
            // Step 2: 根据当前节点和方向计算邻居节点的坐标，并将其加入 neighbors
                // your code
            neighbors.push_back(Node(current.x + dir.first, current.y + dir.second, 0.0, 0.0));
            // finish
        }
...
int main(int argc, char** argv) {
    ...
    ros::NodeHandle nh;
    double map_min_, map_max_, grid_resolution_, safety_radius_;
    ...
    nh.param("astar_planner/safety_radius", safety_radius_, 0.2);
    ...
    ros::Subscriber obstacle_sub = nh.subscribe<visualization_msgs::MarkerArray>("obstacles", 1,
                                                                                 [&planner, &grid_resolution_, &map_min_, &safety_radius_](const visualization_msgs::MarkerArray::ConstPtr& msg) {
                                                                                     for (const auto& marker : msg->markers) {
                                                                                         planner.setObstacle(marker.pose.position.x, marker.pose.position.y, marker.scale.x / 2.0, safety_radius_);
                                                                                     }
                                                                                 });
